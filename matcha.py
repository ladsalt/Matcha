...
def read_matcha_config():
    """Reads the 'matcha-config' file from the script's directory and returns the parsed JSON."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_file_path = os.path.join(script_dir, "matcha-config")
    try:
...
def read_project_config(dir_path):
    """Reads the 'matcha' file from the specified directory and returns the parsed JSON."""
    config_file_path = os.path.join(dir_path, "matcha")
    try:
        with open(config_file_path, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
...
def run_command(project_config, matcha_config, dir_path):
    ...
    ignore = matcha_config.get("ignore", {})
    ...
    for venv_name in venv_configs:
        ...
        try:
            subprocess.run([sys.executable, "-m", "venv", os.path.join(dir_path, venv_name)], check=True)
    ...
    for package, details in dependencies.items():
        ...
        if venv_name and venv_name in venv_configs:
            pip_executable = os.path.join(dir_path, venv_name, "bin", "pip") if os.name != 'nt' else os.path.join(dir_path, venv_name, "Scripts", "pip.exe")
    ...
    if main_venv:
        if os.name == 'nt':  # Windows
            activate_script = os.path.join(dir_path, main_venv, "Scripts", "activate")
            run_command = f'"{activate_script}" && python "{os.path.join(dir_path, project_source)}"'
            try:
            ...
            activate_script = os.path.join(dir_path, main_venv, "bin", "activate")
            run_command = f'source "{activate_script}" && python "{os.path.join(dir_path, project_source)}"'
            try:
...
def main():
    parser = argparse.ArgumentParser(description="Process 'matcha' project configuration.")
    parser.add_argument("--dir", type=str, default=os.path.dirname(os.path.abspath(__file__)), help="Specify the directory containing 'matcha'.")
    ...
    dir_path = args.dir
    matcha_config = read_matcha_config()
    project_config = read_project_config(dir_path)

    if args.command == "info":
        info_command(project_config)
    elif args.command == "run":
        run_command(project_config, matcha_config, dir_path)
    else:
        parser.print_help()
